...
- Name: auth.spec.ts >> navigating back from signup to home does not break navigation
- Location: tests/auth.spec.ts:52:1
...
Error: strict mode violation: locator('nav') resolved to 2 elements:
    1) <nav class="hidden md:flex items-center gap-1">…</nav> aka getByRole('navigation')
    2) <nav class="md:hidden fixed bottom-0 inset-x-0 z-40 border-t border-border bg-background/95 backdrop-blur-md">…</nav> aka locator('nav').nth(1)
